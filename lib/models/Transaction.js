 ...
 */
  static compileSource(source) {
    ...
    Object.keys(compiled.contracts).forEach((k) => {
      ret.contracts.push({
        contract: k.split(':')[1],
        bytecode: compiled.contracts[k].bytecode,
        abi: JSON.parse(compiled.contracts[k].interface)
      });
    });
    (compiled.errors || []).forEach((e) => {
      if (e.indexOf('Warning:') > -1) {
        ret.warnings.push(e);
      } else {
        ret.errors.push(e);
      }
 ...
 * @name compile
 * @memberOf Transaction
 * @param {object} input - Full Solidity JSON structure. See {@link https://solidity.readthedocs.io/en/develop/using-the-compiler.html#compiler-input-and-output-json-description|Compiler Input and Output JSON Description}.
