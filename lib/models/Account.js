 ...
 * @param {number} tokens - The number of tokens to send.
 * @returns {Transaction} Returns a transaction which has already been sent.
 * @api public
 */
  sendTokens(to, tokens) {
    assert.isPositiveNumber(tokens, messages.TRANSACTION_TOKENS_ISPOSITIVENUMBER);
 ...
 * @param {number} tokens - The number of tokens to seed the contract with.
 * @returns {Transaction} Returns a transaction which has already been sent.
 * @api public
 */
 ...
 * @param {number} tokens - The number of tokens to send to the contract for the method call.
 * @returns {Transaction} Returns a transaction which has already been sent.
 * @api public
 */
  executeContract(to, method, params, abi, tokens) {
    ...
    assert.isArray(abi, messages.TRANSACTION_ABI_ISARRAY);
    if (tokens !== undefined) {
      assert.isNumberOrNull(tokens, messages.TRANSACTION_TOKENS_ISNUMBER);
    }
